...
- `app/business/`, `app/news/`, `app/careers/`: それぞれの一覧ページを置く App Router のページ群です。
...
- `next.config.ts`: Next.js の設定ファイルです。このアプリでは静的 HTML 出力、Strict Mode、Turbopack のルート設定をしています。
...
2. 一覧ページは `app/business/` などを開いて、App Router のディレクトリ構造に慣れる
...
- ルートページは `out/index.html`、たとえば事業ページは `out/business/index.html` のように確認できます。
